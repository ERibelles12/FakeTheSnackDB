...
import static de.pecus.api.encryption.helper.Decryption.blockDecrypt;
import static de.pecus.api.encryption.helper.Encryption.blockEncrypt;
import static de.pecus.api.encryption.helper.KeyWrapper.makeKey;
...
import de.pecus.api.encryption.helper.Codecs;
...
//			byte[] encryptedValue = encrypt(value.getBytes("UTF8"), SpecificImplementation.Algorithms.twoFish_makeKey(convertKey(key)));
			byte[] encryptedValue = encrypt(value.getBytes("UTF8"), makeKey(convertKey(key)));
			if (safeEncoding)
...
//	}
	...
	public static String tryDecrypt64(String encryptedOrDecryptedValue, String key) {
		...
		int checkSumLength = EncryptionUtil.getCheckSumLength();
		if (encryptedOrDecryptedValue.length() > checkSumLength) {
			String dec = EncryptionUtil.decrypt64(encryptedOrDecryptedValue, key);
			...
			if (dec.length() >= checkSumLength) {
				...
				if (checksum.equals(EncryptionUtil.checksum(decryptedValue, EncryptionUtil.getCheckSumLength()))) {
...
//			return CommonUtil.rtrim(new String(decrypt(decoded, SpecificImplementation.Algorithms.twoFish_makeKey(convertKey(key))), "UTF8"));
			return CommonUtil.rtrim(new String(decrypt(decoded, makeKey(convertKey(key))), "UTF8"));
		}
		catch (InvalidKeyException e)
		{
...
//			System.arraycopy(SpecificImplementation.Algorithms.twoFish_BlockEncrypt(input_copy, idx * 16, key), 0, output, idx * 16, 16);
			System.arraycopy(blockEncrypt(input_copy, idx * 16, key), 0, output, idx * 16, 16);
		}
   ...
   private static String toString (byte[] ba, int offset, int length) {
	...
	public static byte[] decrypt(byte[] input, Object key)
	{
		...
		for (int idx = 0; idx < count; idx++)
		{
			System.arraycopy(blockDecrypt(input, idx * 16, key), 0, output, idx * 16, 16);
		}
